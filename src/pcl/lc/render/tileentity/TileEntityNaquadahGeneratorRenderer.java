...
public class TileEntityNaquadahGeneratorRenderer extends TileEntitySpecialRenderer {
	...
	public void renderTileEntityAt(TileEntity tileEntity, double x, double y, double z, float f) {
		if (LanteaCraft.getProxy().isUsingModels()) {
			...
			GL11.glTranslatef(-0.5F, -0.5F, -0.5F);
			if (gcewing.sg.tileentity.TileEntityNaquadahGenerator.isActive) {
				theTexture = new ResourceLocation("pcl_lc", "textures/models/naquada_generator_on_" + LanteaCraft.getProxy().getRenderMode() + ".png");
			} else {
				theTexture = new ResourceLocation("pcl_lc", "textures/models/naquada_generator_off_" + LanteaCraft.getProxy().getRenderMode() + ".png");
			}
			FMLClientHandler.instance().getClient().renderEngine.bindTexture(theTexture);
			LanteaCraft.Render.modelNaquadaGenerator.renderAll();
