...
public class ComputerCraftLegacyAgent implements IIntegrationAgent {
	...
	private class ComputerCraftProvider implements IPeripheralHandler {
		...
		public IHostedPeripheral getPeripheral(TileEntity entity) {
			if (entity instanceof IStargateAccess)
				return new ComputerCraftLegacyWrapperPool.StargateAccessWrapper((IStargateAccess) entity);
			else if (entity instanceof IStargateControllerAccess)
				return new ComputerCraftLegacyWrapperPool.StargateControllerAccessWrapper(
						(IStargateControllerAccess) entity);
			else if (entity instanceof INaquadahGeneratorAccess)
				return new ComputerCraftLegacyWrapperPool.NaquadahGeneratorAccessWrapper(
						(INaquadahGeneratorAccess) entity);
			else
				return null;
		}
	}
	...
	public ComputerCraftLegacyAgent() {
