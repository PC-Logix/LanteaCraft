...
public class AnalyticsHelper extends Thread {
	...
	public void run() {
		...
		while (tries < maxTries)
			try {
				tries++;
				LCLog.debug("Pushing metrics data (try " + tries + " of " + maxTries + ")");
				push();
				LCLog.debug("Done submitting anonymous data.");
				return;
			} catch (IOException ioex) {
				LCLog.warn("The metrics push failed.", ioex);
			}
