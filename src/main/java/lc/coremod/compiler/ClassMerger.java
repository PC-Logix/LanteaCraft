...
public class ClassMerger {
	...
	public static LCCompilerException[] mergeClasses(ClassNode source,
			ClassNode destination, boolean force) {
	...
	private static void walkMergePreconditions(ClassNode source,
			ClassNode dest, List<LCCompilerException> errors) {
		if (source.fields != null && source.fields.size() != 0) {
			for (FieldNode field : source.fields)
				if (hasDuplicateField(field, dest))
					errors.add(new LCCompilerException("Duplicate field: "
							+ ASMAssist.signature(field)));
		}

		if (source.interfaces != null && source.interfaces.size() != 0) {
			for (String iface : source.interfaces)
				if (hasDuplicateInterface(iface, dest))
					errors.add(new LCCompilerException("Duplicate interface: "
							+ iface));
		}
		...
		if (source.methods != null && source.methods.size() != 0) {
			for (MethodNode srcMethod : source.methods) {
				if (hasDuplicateMethod(srcMethod, dest)) {
					if (ASMAssist.isMethodEmpty(srcMethod))
						continue;
					MethodNode dstMethod = ASMAssist.findMethod(dest,
							srcMethod.name, srcMethod.desc);
					if (ASMAssist.isMethodEmpty(dstMethod))
						continue;
					errors.add(new LCCompilerException(
							"Duplicate non-empty method import: "
	...
	private static boolean hasDuplicateMethod(MethodNode theMethod,
			ClassNode theClass) {
		return (ASMAssist.findMethod(theClass, theMethod.name, theMethod.desc) != null);
	}

	private static boolean hasDuplicateField(FieldNode theField,
			ClassNode theClass) {
		return (ASMAssist.findField(theClass, theField.name) != null);
	}

	private static boolean hasDuplicateInterface(String theInterface,
			ClassNode theClass) {
	...
	private static FieldNode remapField(String sourceName, String destName,
			FieldNode master) {
		return master;
	}
