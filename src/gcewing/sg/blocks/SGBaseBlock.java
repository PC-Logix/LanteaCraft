...
import gcewing.sg.core.SGState;
...
public class SGBaseBlock extends Base4WayCtrBlock {
	...
	public boolean isMerged(IBlockAccess world, int x, int y, int z) {
		SGBaseTE te = (SGBaseTE) getTileEntity(world, x, y, z);
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float cx,
		...
		if (te != null) {
			...
			if (te.isMerged) {
				SGCraft.getInstance().openGui(player, SGGui.SGBase, world, x, y, z);
				return true;
			}
		}
		return false;
