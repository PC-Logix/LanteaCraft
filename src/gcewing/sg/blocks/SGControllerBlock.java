...
public class SGControllerBlock extends Base4WayCtrBlock {
	...
	public void breakBlock(World world, int x, int y, int z, int id, int data) {
		SGControllerTE cte = (SGControllerTE) getTileEntity(world, x, y, z);
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float cx,
			float cy, float cz) {
		SGCraft.getInstance().openGui(player, SGGui.SGController, world, x, y, z);
		return true;
	}

	public void checkForLink(World world, int x, int y, int z) {
		((SGControllerTE) getTileEntity(world, x, y, z)).checkForLink();
	}

	@Override
	public TileEntity createNewTileEntity(World world) {
		return new SGControllerTE();
	}
